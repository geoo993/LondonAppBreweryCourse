...
struct RxAnimated {
    static let areAnimationsEnabled = Variable(true)
    fileprivate static var areDefaultHeuristicsEnabled = false
    static func enableDefaultPerformanceHeuristics() {
...
struct AnimationType<Base> {
     ...
     */
    init(type: RxAnimationType, duration: TimeInterval, options: UIView.AnimationOptions = [], setup: ((UIView) -> Void)? = nil, animations: ((UIView) -> Void)?, completion: ((Bool) -> Void)? = nil) {
        self.type = type
...
//    func bind(animated observer: Binder<E>) -> Disposable {
...
struct AnimatedSink<Base> {
    var type: AnimationType<Base>!
    var base: Base
