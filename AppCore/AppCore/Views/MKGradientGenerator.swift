...
class MKGradientGenerator {
    class func gradientImage(
        type: GradientType, size: CGSize, colors: [CGColor], colors2: [CGColor]? = nil,
        locations: [Float]? = nil, locations2: [Float]? = nil,
        startPoint: CGPoint? = nil, endPoint: CGPoint? = nil,
        startPoint2: CGPoint? = nil, endPoint2: CGPoint? = nil, scale: CGFloat? = nil) -> CGImage {
        ...
        for y in 0 ..< h {
            for x in 0 ..< w {
                let c = pixelDataForGradient(
                    type, colors: [colors, colors2], locations: [locations, locations2],
                    startPoints: [startPoint, startPoint2], endPoints: [endPoint, endPoint2],
                    point: CGPoint(x: x, y: y), size: CGSize(width: w, height: h))
                data.append(c)
            }
        }

        let ctx = CGContext(data: &data, width: w, height: h, bitsPerComponent: bitsPerComponent,
                            bytesPerRow: w * bytesPerPixel, space: colorSpace, bitmapInfo: bitmapInfo.rawValue)
    ...
    fileprivate class func pixelDataForGradient(
        _ gradientType: GradientType, colors: [[CGColor]?], locations: [[Float]?],
        startPoints: [CGPoint?], endPoints: [CGPoint?], point: CGPoint, size: CGSize) -> RGBA {
...
private func == (lhs: RGBA, rhs: RGBA) -> Bool {
...
private func lerp(_ value: Float, inRange: ClosedRange<Float>, outRange: ClosedRange<Float>) -> Float {
    return (value - inRange.lowerBound) * (outRange.upperBound - outRange.lowerBound)
        / (inRange.upperBound - inRange.lowerBound) + outRange.lowerBound
}
